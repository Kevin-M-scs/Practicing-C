#include <stdio.h>
#include <stdlib.h>
//addition of sparse matrices, custom solution
...
int main(){
    int row1,col1;int count1,count2;//count its number of non zero elements
    int row2,col2;
...
int * add(int *ptr1,int *ptr2,int count1,int count2,int row,int col,int *ct3){
    ...
    *(sum)=row;*(sum+1)=col;
    ...
    if(i>count1&&j<=count2){
        for(j;j<=count2;j++){
            push(&sum,++c3,*(ptr2+(j*3)),*(ptr2+(j*3)+1),*(ptr2+(j*3)+2));
        }
    }
    if(j>count2&&i<=count1){
        for(i;i<=count1;i++){
            push(&sum,++c3,*(ptr1+(i*3)),*(ptr1+(i*3)+1),*(ptr1+(i*3)+2));
        }
    }
    *(sum+2)=c3;
    *ct3=c3;
